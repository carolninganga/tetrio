...
# 3/16/2021: updated getFill to fix a bug in that it returned only two numbers
#            getFill also returns just the current fill value, it doesn't break
#            it into three numbers.  That way it can be passed directly back into
#            a setFill function.
#            added a color_split function that takes in a #RRGGBB color specifier
#            and returns the RGB values as a tuple.
#            Bruce Maxwell
...
color name string, it returns the name string.
        ...
        Updated to return the value with no modification so it can be used directly in setFill
        Should create a gr.color_num that given a color returns the RGB values

        """
        options = self.config # BAM don't assume canvas is not None, use the config dictionary
        val = options['fill']
...
color name string, it returns the name string.
    ...
    def drawn(self):
       "Return true if self.canvas is not equal to None"
...
def color_split( colorString ):
   """Given a color specifier string of the form #RRGGBB, it splits the string
      and returns the three integer values in the range (0, 255) corresponding
      to the r, g, b intensities.  If given an invalid input, it returns the
      integers (0, 0, 0)"""
